...
用于将 ArXiv 论文摘要、博客文章等翻译/总结成简体中文
"""
import sys
import time
import logging
...
from src.config import LLM_API_KEY, LLM_API_URL, LLM_MODEL, LLM_TIMEOUT, LLM_MAX_RETRIES


def _call_llm(prompt: str, temperature: float = 0.3, max_tokens: int = 256,
...
{text}"""
    ...
    for attempt in range(LLM_MAX_RETRIES):
        result = _call_llm(prompt, temperature=0.3, max_tokens=1024)
        if result:
            return result
        if attempt < LLM_MAX_RETRIES - 1:
            logger.warning(f"LLM 翻译返回空，重试 ({attempt + 1}/{LLM_MAX_RETRIES})...")
            time.sleep(2 ** attempt)

    logger.error("LLM 翻译最终失败")
    return text[:max_chars] + "..." if len(text) > max_chars else text
...
def generate_brief(content: str, category: str = "general") -> str:
    ...
    """
    if not LLM_API_KEY or not content or len(content) < 20:
        return ""
...
{content[:3000]}"""
    ...
    return _call_llm(prompt, temperature=0.5, max_tokens=256)
...
{content[:6000]}"""
    ...
    return _call_llm(prompt, temperature=0.4, max_tokens=max_tokens)
...
def generate_news_brief(title: str, content: str = "", category: str = "tech",
    ...
    Args:
        ...
        _depth: 内部递归计数器，防止 AI 持续返回 [JUNK] 时无限递归

    Returns:
        中文短报（80-120字），失败则返回空字符串
    """
    if not LLM_API_KEY or not title:
        return ""
...
6. 直接输出摘要，不要前缀。"""
    ...
    result = _call_llm(prompt, temperature=0.3, max_tokens=max_tokens)
...
def expand_product_tagline(name: str, tagline: str) -> str:
    ...
    """
    if not LLM_API_KEY or not name:
        return ""
...
请用一句自然中文描述这个产品的定位和卖点（30-60字）。直接输出，不要前缀。"""
    ...
    return _call_llm(prompt, temperature=0.4, max_tokens=128)
